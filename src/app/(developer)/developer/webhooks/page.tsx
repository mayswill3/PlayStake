...
export default function WebhooksPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-surface-100">Webhooks</h1>
            ...
            <Link key={game.id} href={`/developer/games/${game.id}`}>
              <Card className="hover:border-surface-700 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-surface-100">{game.name}</p>
                    <p className="text-xs text-surface-500 mt-0.5">{game.slug}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    {game.webhookUrl ? (
                      <>
                        <code className="text-xs text-surface-400 font-mono bg-surface-800 px-2 py-1 rounded max-w-[200px] truncate">
                          {game.webhookUrl}
                        </code>
      ...
      <Card>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            ...
            <div key={item.event} className="p-3 rounded-lg bg-surface-800">
